...
public class ProcessScheduler extends JFrame { // Process Scheduler GUI
      ...
      private JTable JobPool;
      private JTable gant_table;
      private JTable PN_table;
   
      public static Object[][] rowData = null; // Job Pool 테이블 인덱스
      public static Object[][] gant_x = null;
      public static Object[][] PN_x = null;
      ...
      public static ArrayList arrival = new ArrayList(); //프로세스 도착시간들을 받는 배열
      public static ArrayList burst = new ArrayList(); //프로세스 수행시간들을 받는 배열
      public static double[] waitArr = null; //프로세스 대기시간들을 받는 배열
      public static double[] turnArr = null;
      public static double[] NTTArr = null;
      public static int[] orderArr = null;
      public static int[] startArr = null;
      public static int[] endArr = null;
      public static int[] paintArr = null;
      ...
      public ProcessScheduler() {
         ...
         JLabel lblNewLabel_4 = new JLabel("<html>Process_name, BT, AT <br>\uC785\uB825\uC2DC \uB744\uC5B4\uC4F0\uAE30\uB85C \uAD6C\uBD84\uD574 \uC8FC\uC138\uC694 <br>PN,AT,BT\uB294 insert, WT,TT,NTT\uB294<br> start \uBC84\uD2BC\uC744 \uB204\uB974\uBA74 \uCD9C\uB825\uB429\uB2C8\uB2E4.<br>PN &nbsp p1  p2 p3 p4 <br>AT &nbsp 1 &nbsp 2 &nbsp 3 &nbsp 4 <br>BT &nbsp 3 &nbsp 4 &nbsp 2 &nbsp 2");
         ...
         btnInsert.addActionListener(new ActionListener() { // Insert버튼 구현
            public void actionPerformed(ActionEvent e) {
             ...
             qts = quantum_tmp.toCharArray();
              ...
              for (int i = 0; i < quantum_tmp.length(); i++) {
                    if (!('0' <= qts[i] && '9' >= qts[i] || qts[i] == ' ')) {
              ...
              if(algorithmBox.getSelectedIndex()==1 || algorithmBox.getSelectedIndex()==5) {//Round Robin 이 아닌경우 timequantum 입력을 않받음
                  if(quantum_tmp.isEmpty() || Integer.parseInt(quantum_tmp) == 0) {
                    JOptionPane.showMessageDialog(null, "TQ is not entered \n press reset!", "Error", JOptionPane.WARNING_MESSAGE);
                    btnStart.hide();
                  }
              //quantum는 Double.parseDouble(tq)
              }
              else {
...
//                  System.out.println(ordem);
      ...
      public void SPN_func()
      {
         ...
         startArr = new int [start.size()];
         endArr = new int [end.size()];
         orderArr = new int [order.size()];
         paintArr = new int [paint.size()];
         
         for(int i=0;i<order.size();i++)
            orderArr[i]=(int)order.get(i);
         for(int i=0;i<start.size();i++)
            startArr[i]=(int)start.get(i);
         for(int i=0;i<end.size();i++)
            endArr[i]=(int)end.get(i);
         for(int i=0;i<paint.size();i++)
            paintArr[i]=(int)paint.get(i);
         
//         System.out.println("\tat\tbt\tct\tta\twt\tntt");
         for(int i=0;i<N;i++)
         {
            avgwt+= waitArr[i];
            avgta+= turnArr[i];
//            System.out.println("P"+pid[i]+"\t"+at[i]+"\t"+bt[i]+"\t"+ct[i]+"\t"+ta[i]+"\t"+wt[i]+"\t"+Math.round(((float)ta[i] / bt[i])*100.0)/100.0);
         }
         ...
         public void RRR_func() {
             ...
             while(quantityProcesses > 0)
             {
                       ...
                       else
                       {
                           ...
                           {
                              paint.add(execution);
                              //paintArr[currentTime] = execution;
                               currentTime++;
                               q--;
                               burstUpdate = (int) burst.get(execution) -1;
                               burst.set(execution, burstUpdate);
                   ...
                   middleHold = 0;
    //               System.out.println("Processing - PART" + contTeste);
